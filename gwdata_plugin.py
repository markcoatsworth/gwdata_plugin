...
def parse_args():
    ...
    parser.add_argument("-classad", action="store_true",
                        help="Output classad of options supported by this "
                        "transfer plugin")
    parser.add_argument("-infile", type=str, action="store",
                        help="Input file of classads describing files to "
                        "transfer")
    parser.add_argument("-outfile", type=str, action="store",
                        help="Output file of classads describing file "
                        "transfer results")
    ...
    def create_cache(self, args, urls):
        ...
        if cache == "lal" or cache == "lal-cache":
            ...
            for url in urls:
                ...
                # If this is the first url in the list, set tracking variables
                if url == urls[0]:
            ...
            metadata_file.write(
                "Error: {0} is not a valid cache type. Allowed types are "
                "\"lal\", \"lal-cache\", \"frame\" and \"frame-cache\".\n"
                .format(cache)
            )
    ...
    try:
        with open(args['outfile'], 'w') as outfile:
            for ad in infile_ads:
                try:
                    result_ads, transfer_success = gwdata.download_data(
                        ad['Url']
                    )
